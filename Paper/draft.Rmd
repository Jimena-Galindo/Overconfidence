...
A growing body of literature in economics studies the mechanisms through which economic agents come to 
hold incorrect beliefs about a fundamental. Most of this literature focuses on the case in which agents
are passive observers of the world and incorrectly incorporate the information they receive into their 
beliefs\footnote{See \citet{benjamin2019} for a review of error in probabilistic thinking}.
However, in many situations, agents are active participants in the process of generating information.
In such cases, the information they receive is affected by their actions and the way they process it
can affect their future behavior. I refer to this as an \emph{endogenous information} setting.
This sort of setting, has been shown to explain pehnomena that
cannot be rationalized with the classical models of biased belief updating (e.g. @Nyarko1991 and 
@Levy2022). Settings with endogenous information and misspecified learning also offer alternative explanations 
for why certain well known biases might persist. I focus on a group of theories that aim to explain 
the prevalence of overconfidence. 

Overconfidence has been shown to be a prevalent bias in a variety of settongs. @Oster2013 shows that 
subjects who are at high risk of having Huntingtons disease underestimate their probability of 
having the disease and make retirement decisions as if they were not sick. Relating to investment 
decisions, @Hoffman2020 show that workers overestimate their productivity in their 
current employment and are unlikely to look for other oportunities. @Camerer1999 find that 
entrepreneurs are overconfident about the quiality of their enterprise, 
which leads to excessive entry and early exit from markets. These are only some examples of how 
holding an incorrect belief about a fundamental can lead to costly decisions \footnote{Other examples 
of overconfidence are }.

I isolate the main forces from 3 different theories of misspecified learning. Each of the theories 
rationalizes the prevalence of overconfidence in frameworks where the agents are learning about 
an ego-relevant parameter and an exogenously determined state. The interaction between these two 
parameters together with an endogenous information process give rise to two explanaitions
for the presistence of overconfidence. On one hand, even with bayesian updating, the agent 
may fall into learning traps. Learning traps are misspecified beliefs and optimal actions 
that induce signals that confirm the misspecified belief. Once an agent finds themselves in 
such a situation, the belief will be stable and even with a correctly speficified model of learning, 
they will not be able to abandon their icorrect beliefs. On the other hand a setting with two variables allows the agent to engage in asymmetric updating 
with respect to good and bad news. When the outcome depends on an ego-relevant parameter as well
as a state, the agent can attribute good news to a high value of the parameter while attributing 
failures to a low value of the exogenous parameter\footnote{This is in line with the well known theory 
in behavioral science of self-attribution bias of which \citet{kelley1980} provides a review.}.

@Heidhues2018 shows 
that under certain assumptions on the output function, learning traps can rationalize the 
existance of misspecified beliefs when agents are dogmatic about the ego-relevant parameter. 
In a more general model @Ba2023 further analyzes this setting 
and characterizes the stuations in which even agents who are not dogmatic might become trapped.
She allows initi dogmatic agents to consider alternative theories and adopt whichever is a 
better explanation of the observed data. They do so by performing Bayesian hypothesis tests
that compare the likelihood of having observed the outcome under alternative paradigms\footnote{A 
similar mechanism is proposed by \citet{Schwarstein2021} in a setting with persuasion}. Although
this learning mechanism allows agents to avoid the learning traps, there are still situations under
which misspecified beliefs may persist. 
